...
describe('addKey', function() {
  describe('unary functions like `map`', function() {
    ...
    var addKeyParam = function(x, idx) {return x + idx;};
    ...
    it('works just like a normal map', function() {
      eq(mapKeyed(times2, [1, 2, 3, 4]), [2, 4, 6, 8]);
      eq(mapKeyed(times2, { a:1, b:2, c:3 }), { a:2, b:4, c:6 });
    });

    it('passes the index as a second parameter to the callback', function() {
      eq(mapKeyed(addKeyParam, [8, 6, 7, 5, 3, 0, 9]), [8, 7, 9, 8, 7, 5, 15]); // [8 + 0, 6 + 1...]
      eq(mapKeyed(addKeyParam, { a: 'foo', b: 'bar' }), { a: 'fooa', b: 'barb' });
    });

    // it('passes the entire list as a third parameter to the callback', function() {
    //   eq(mapKeyed(squareEnds, [8, 6, 7, 5, 3, 0, 9]), [64, 6, 7, 5, 3, 0, 81]);
    // });

    // it('acts as a curried function', function() {
    //   var makeSquareEnds = mapKeyed(squareEnds);
  ...
  //   var reduceKeyed = R.addKey(R.reduce);
  //   var timesKeyed = function(tot, num, idx) {return tot + (num * idx);};
  ...
  //     eq(reduceKeyed(timesKeyed, 0, [1, 2, 3, 4, 5]), 40);
  //     eq(reduceKeyed(objectify, {}, ['a', 'b', 'c', 'd', 'e']), {a: 0, b: 1, c: 2, d: 3, e: 4});
  //   });

  //   it('passes the entire list as a fourth parameter to the predicate', function() {
  //     var list = [1, 2, 3];
  //     reduceKeyed(function(acc, x, idx, ls) {
  //       eq(ls, list);
  //       return acc;
  //     }, 0, list);
  //   });
  ...
  //   var allKeyed = R.addKey(R.all);
  //   var superDiagonal = allKeyed(R.gt);
  ...
  //   };
  //   var mapFilterKeyed = R.addKey(mapFilter);
