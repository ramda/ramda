...
describe('sequence', function() {
  ...
  it('operates on a list of applicatives', function() {
    eq(R.sequence(S.Maybe, [S.Just(3), S.Just(4), S.Just(5)]), S.Just([3, 4, 5]));
    eq(R.sequence(S.Maybe, [S.Just(3), S.Nothing, S.Just(5)]), S.Nothing);
  });

  it('traverses left to right', function() {
    eq(R.sequence(S.Either, [S.Right(1), S.Right(2)]), S.Right([1, 2]));
    eq(R.sequence(S.Either, [S.Right(1), S.Left('XXX')]), S.Left('XXX'));
    eq(R.sequence(S.Either, [S.Left('XXX'), S.Right(1)]), S.Left('XXX'));
    eq(R.sequence(S.Either, [S.Left('XXX'), S.Left('YYY')]), S.Left('XXX'));
  });

  it('dispatches to `fantasy-land/traverse` method', function() {
    eq(R.sequence(Id, [Id(1), Id(2), Id(3)]), Id([1, 2, 3]));
    eq(R.sequence(Array, Id([1, 2, 3])), [Id(1), Id(2), Id(3)]);
  });
