...
describe('extend', function() {
    ...
    it('takes two objects, merges their own properties and returns a new object', function () {
        var a = {w: 1, x: 2};
        var b = {y: 3, z: 4};
        assert.deepEqual(extend(a, b), {w: 1, x: 2, y: 3, z: 4});
    });

    it('overrides properties in the first object with properties in the second object', function () {
        var a = {w: 1, x: 2};
        var b = {w: 100, y: 3, z: 4};
        assert.deepEqual(extend(a, b), {w: 100, x: 2, y: 3, z: 4});
    });

    it('should be destructive', function() {
        var a = {w: 1, x: 2};
        var res = extend(a, {x: 5});
        assert.equal(a, res);
        assert.deepEqual(res, {w: 1, x: 5});
    });
    ...
    it('only own properties', function() {
        ...
        function Cla() {}
        Cla.prototype.x = 5;
        assert.deepEqual(extend(new Cla, a), {w: 1, x: 2});
        assert.deepEqual(extend(a, new Cla), {w: 1, x: 2});
    });

    it('outta be curried', function () {
        var curried = extend({w: 1, x: 2});
        var b = {y: 3, z: 4};
        assert.deepEqual(curried(b), {w: 1, x: 2, y: 3, z: 4});
    });
});
