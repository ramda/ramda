...
describe('where', function() {
  ...
  it('is curried', function() {
    var predicate = R.where({x: R.eq(1), y: R.eq(2)});
  ...
  it('matches inherited properties', function() {
    var spec = {
      toString: R.eq(Object.prototype.toString),
      valueOf: R.eq(Object.prototype.valueOf)
    };
    assert.strictEqual(R.where(spec, {}), true);
  });

  it('does not match inherited spec', function() {
    function Spec() { this.y = R.eq(6); }
    Spec.prototype.x = R.eq(5);
    var spec = new Spec();
