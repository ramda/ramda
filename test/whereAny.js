...
describe('whereAny', function() {
  ...
  it('returns true if the test object satisfies the spec', function() {
    var spec = {x: R.equals(0), y: R.equals(2)};
    var test1 = {x: 0, y: 200};
    var test2 = {x: 0, y: 10};
    var test3 = {x: 1, y: 101};
    var test4 = {x: 1, y: 2};
  ...
  it('is false for an empty spec', function() {
    eq(R.whereAny({}, {a: 1}), false);
  });
  ...
  it('does not match inherited spec', function() {
    ...
    eq(R.whereAny(spec, {y: 6}), true);
    eq(R.whereAny(spec, {x: 5}), false);
  });
